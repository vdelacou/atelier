...
2. **Map each changed file to its rule subset.** The rules are layer-specific — audit only what applies:
   ...
   - `package.json` → 19 (no `"latest"`/`"*"`). Any commit or the git history → 23 (Conventional Commits), 26 (neutral identity: no real name, personal or `@company` email in author/committer metadata or in any blob), and small (≤10 files / ≤300 lines).
...
1. **Assess.** Scan the whole tree and report which rules are violated, where, and how pervasively. Rank by leverage — the toolchain and the test seam before cosmetic rules. Include the git history in the scan: a real name or a personal / `@company` email in the commit trail is a rule 26 leak, remedied by a one-time `git filter-repo` rewrite plus a force-push. Surface that as a gated, destructive step; never run it unprompted (rule 25).
