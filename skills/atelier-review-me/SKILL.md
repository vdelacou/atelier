...
3. **Check the universal hard rules on every file.** No `class` / `function` declaration / `interface` / `console.*` (1-4), explicit return types on exports (6), single-arrow not curried (18), zero inline ignores of any tool (15), and no person, employer, or client named in file contents (26, commit metadata exempt).
