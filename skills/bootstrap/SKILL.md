...
Like grill-me, it is a focused, on-demand counterpart to the always-on atelier standard: grill-me owns the pre-decision moment, bootstrap owns the repo-birth moment.
...
3. **Follow the matching bootstrap checklist verbatim.** Read and execute the numbered checklist for the variant — do not improvise the config:
   - Bun-script → `references/bun-typescript.md` (§ Bootstrap checklist): layout, `tsconfig`, flat ESLint config, `Result` + Logger port/adapter/fake, asset copy, coverage gate, Stryker, the two git hooks, `bunfig.toml`.
   - Next.js → `references/nextjs-monorepo.md` (§ Bootstrap checklist): package layout, `tsconfig`/`eslint`/`postcss`/`next.config`, `simple-git-hooks`, `globals.css`, the `src/components/{atoms,molecules,organisms}` + `src/page` + `src/lib` + `src/config` tree.
