...
| Pre-commit | Fast-gate `.githooks/pre-commit` (full set in `ci.yml`) | `simple-git-hooks`: test + lint + commitlint, never install both hook mechanisms | Shell hook: size → pom sanity → gitleaks → `spotless:check` → `./mvnw verify` → PIT |
