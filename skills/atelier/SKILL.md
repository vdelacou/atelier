...
26. **No identifying author or company context in the repo.** Keep real names, personal or employer emails, and company references out of tracked files, docs, commit metadata, and git history. Commit under a neutral repo-local identity (`git config --local user.name` and `user.email`, e.g. `atelier <atelier@users.noreply.github.com>`), never your global one, and make the copyright holder the project or a pseudonym rather than a legal name. Git history is forensic and permanent. A single `@company.com` author email or a real name in one commit leaks affiliation for good, and a later force-push does not purge commits the host has already cached. Set the neutral identity at repo birth (atelier-greenfield does it before commit one), audit for leaks before every publish or hand-off (atelier-review-me), and if something already leaked, rewrite history with `git filter-repo` and force-push, treating the old commits as still exposed until the host garbage-collects. Neutral by default trades git-blame attribution for privacy; a repo that genuinely needs per-author accountability opts back in explicitly, the same way rules 24 and 25 gate on the user's say-so. (Behavioural gate, like rules 24 and 25. The discipline is the enforcement.)
...
- A real name, a personal or `@company` email, or a company reference reaching the repo's tracked files, commit metadata, or git history without a neutral identity in place (rule 26). Set a repo-local neutral `user.name` and `user.email` before the first commit, audit before every publish, and if one already shipped, rewrite with `git filter-repo`, force-push, and treat the cached commits as still exposed.
