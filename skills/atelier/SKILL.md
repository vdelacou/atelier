...
14. **Outside-in classicist TDD.** The System Under Test is the **primary port** (use case, command handler, application service), never an individual entity, value object, or domain service. Entities, value objects, and domain services are used **real** in tests. Only **secondary ports** (repository, email sender, clock, token decoder) get hand-written fakes. Every test name describes a complete business scenario in domain language. This keeps the domain free to refactor without breaking tests. One exception, kept rare: a value object or domain service whose own logic is non-trivial (currency arithmetic, tier brackets, range overlap) earns a few small direct tests that supplement the port tests and never replace them; a regex-shaped id does not qualify, and the shipped `assets/format-error.test.ts` is that exception in practice. See `references/testing.md` (Value-object tests). Inspired by Ian Cooper's *TDD, Where Did It All Go Wrong?*. See `references/testing.md`.
...
- `coverageThreshold` set in `bunfig.toml` while a per-tier script owns enforcement. Bun exits non-zero on the global threshold before the script can print per-file violations, which looks like "coverage failed silently". Remove the global threshold; let the script own it.
