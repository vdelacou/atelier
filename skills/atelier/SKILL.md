...
20. **Bun file API in production; `node:fs` only in tests; `node:path` anywhere.** All filesystem IO in `src/**` production code goes through `Bun.file(path).text() / .json() / .arrayBuffer() / .exists()` and `Bun.write(path, ...)`. Never `node:fs` (`readFileSync`, `writeFile`, `fs/promises`, etc.) inside `src/**`. `node:fs` IS allowed in `*.test.ts` and `src/test-helpers/**` for real-temp-dir setup (`mkdtempSync`, `writeFileSync`, `rmSync`) and for forcing error branches in FS adapters (`chmodSync` on a real file or directory) — `Bun.file` has no `mkdtemp` equivalent and `Bun.file(dir).exists()` returns `false` for directories, so you cannot force the throw path with it. `node:path` (`join`, `dirname`, `resolve`, `basename`) is allowed anywhere — it is path manipulation, not IO. Reason: keeping production on `Bun.file` is faster, has zero `import` ceremony, fits the `try/catch`-quarantine-in-`infra/**` pattern cleanly, and lets the project disable `security/detect-non-literal-fs-filename` at the lint level without losing real coverage (the rule does not watch `Bun.file`). See `references/result-type.md`, `references/testing-infra.md` (filesystem patterns), `references/workflow.md` (lint-rule rationale).
...
- A `node:fs` import (`readFile`, `writeFile`, `readFileSync`, `fs/promises`, etc.) in any file under `src/**` that is not a `*.test.ts` or under `src/test-helpers/**`. Production filesystem IO uses `Bun.file` / `Bun.write`. Hard rule 20.
