...
- Baseline run (COSTLY): run.sh spawns `claude -p` per task per arm (20 runs for e1-e10 alone). A
  prior discipline baseline exists (LESSONS: with-skill 14/14 vs baseline 11/14, gitignored).
  Re-run to record a fresh rule-keyed baseline once the task set is final. Confirm before running.
