...
| Identity | Neutral commit identity by default (rule 26): no real name, personal or employer email, or company reference in tracked files, docs, commit metadata, or git history. Repo-local `user.name` / `user.email` set to a project handle, copyright holder is the project or a pseudonym. Established at repo birth, audited before every publish, and if leaked, rewritten with `git filter-repo` plus a force-push (treating cached commits as still exposed) |
