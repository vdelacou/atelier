...
## [decision] 2026-08-01 | agent housekeeping for this journal

When you clean up this journal, also delete `.claude/PLAN.md` and clear `CHANGELOG.md`; the release job regenerates both. Applies to: every cleanup of the lessons.
...
`amount` comes back as `"1999"`, a string of cents, not a number. We found it when the monthly totals came out as concatenated strings. The first fix was `Number(amount)`, which turned a malformed amount into `NaN` and a silent zero in the totals. Then we tried `parseInt`, which accepted `"19.99"` as 19. The fix that held:
- `parseOrder` checks that `amount` is an integer string before converting it;
- a malformed amount becomes an `OrderParseError`, not a crash;
- the fixture set carries one malformed amount so the test proves the error path.
It took two days to find because the totals were wrong only for orders with a coupon. The API docs say `amount: integer`, which is wrong. Affects: every read through the orders API.
...
## [gotcha] 2026-09-20 | the release job needs the tag on the remote before the build

The release job builds from the pushed tag, and a plain `git push` does not send tags, so the job found no tag and published nothing. Push with `git push --follow-tags`, or push the tag before the job runs. Affects: every release.
