...
Three checks, any failure exits non-zero (a CI gate):
  1. The matrix has exactly 120 rows, with the canonical per-pillar counts.
...
PER_PILLAR = [3, 6, 9, 9, 10, 7, 7, 6, 5, 14, 3, 7, 5, 3, 10, 5, 7, 4]
...
def check(matrix_text: str, canon_text: str) -> list[str]:
    ...
    if len(rows) != 120:
        errors.append(f"row count is {len(rows)}, expected 120")
