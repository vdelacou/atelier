...
Three checks, any failure exits non-zero (a CI gate):
  1. The matrix has exactly 117 rows, with the canonical per-pillar counts.
...
PER_PILLAR = [3, 6, 9, 8, 10, 7, 7, 6, 5, 14, 3, 7, 4, 3, 9, 5, 7, 4]
...
def check(matrix_text: str, canon_text: str) -> list[str]:
    ...
    if len(rows) != 117:
        errors.append(f"row count is {len(rows)}, expected 117")
...
def main() -> None:
    ...
    if errors:
        ...
        for e in errors:
            print(f"  {e}")
        sys.exit(1)
    print("conformance-matrix.md: 117 rows, ids+titles verbatim, canon sha256 pins intact")
